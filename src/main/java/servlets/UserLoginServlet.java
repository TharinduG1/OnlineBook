...
public class UserLoginServlet extends GenericServlet {
	public void service(ServletRequest req, ServletResponse res) throws IOException, ServletException {
		...
		String safeInput = Encode.forHtml(uName);
		try {

			MessageDigest digest = MessageDigest.getInstance("SHA-256");
			byte[] hashBytes = digest.digest(pWord.getBytes());
			pWord = Base64.getEncoder().encodeToString(hashBytes);
			...
			if (rs.next()) {
				...
				rd.include(req, res);
				pw.println("<div class=\"home hd brown\">Welcome ! " + safeInput + "</div><br/>");
